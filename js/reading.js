...
function shareTabReading(tabId, title) {
  // Build share text from reading data
  const d = readingData;
  let shareText = title;
  if (d) {
    ...
    const parts = [];
    if (astro?.sign) parts.push(astro.symbol + ' ' + astro.sign);
    if (astro?.moon_sign) parts.push('Moon in ' + astro.moon_sign);
    if (hd?.type) parts.push(hd.type);
    if (num?.life_path) parts.push('Life Path ' + num.life_path.number);
    if (parts.length) shareText = parts.join(' | ');
  }

  const url = 'https://aethera.live';
  const fullText = shareText + ' — Discover your cosmic identity';
  ...
  content.innerHTML = `
    ...
    <p style="color:var(--muted);font-size:.85rem;margin-bottom:20px;line-height:1.5">${shareText}</p>
    <div class="share-social-grid">
      <a class="share-social-btn share-facebook" href="https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}&quote=${encodedText}" target="_blank" rel="noopener">
      ...
      <a class="share-social-btn share-twitter" href="https://twitter.com/intent/tweet?text=${encodedText}&url=${encodedUrl}" target="_blank" rel="noopener">
      ...
      <button class="share-social-btn share-instagram" onclick="shareToInstagram()">
      ...
      <a class="share-social-btn share-tiktok" href="https://www.tiktok.com/share?url=${encodedUrl}&text=${encodedText}" target="_blank" rel="noopener">
      ...
      <a class="share-social-btn share-snapchat" href="https://www.snapchat.com/share?url=${encodedUrl}" target="_blank" rel="noopener">
      ...
      <a class="share-social-btn share-whatsapp" href="https://wa.me/?text=${encodedText}%20${encodedUrl}" target="_blank" rel="noopener">
...
function shareToInstagram() {
  // Instagram doesn't support direct URL sharing — guide the user
  const text = 'Screenshot your reading and share it on Instagram Stories! Tag @aethera.live';
  alert(text);
}
